...
__all__ = ["available_models", "load", "tokenize", "encode_text_with_prompt_ensemble",
           "get_similarity_map", "clip_feature_surgery", "similarity_map_to_points", "generate_clip_fts", "reload_visual_attn"]
...
_MODELS = {
    ...
    "ExCEL_ViT-B/16": "https://openaipublic.azureedge.net/clip/models/5806e77cd80f8b59890b7e101eabd078d9fb84e6937f9e85e4ecb61988df416f/ViT-B-16.pt",
}
...
def clip_feature_surgery(image_features, text_features, redundant_feats=None, t=2):
    ...
    if redundant_feats != None:
        ...
        attr_maps = (similarity - similarity.min(1, keepdim=True)[0]) / (similarity.max(1, keepdim=True)[0] - similarity.min(1, keepdim=True)[0])

    return attr_maps.detach()
...
def generate_clip_fts(inputs, model, return_weights=True, ex_feats=None):
    
    model = model.cuda()
    
    image_features, attn_weight_list, feat_list = model.encode_image(inputs, return_weights, ex_feats)
    image_features = image_features / image_features.norm(dim=1, keepdim=True)

    attn_weights = torch.stack(attn_weight_list,dim=0)
    all_feats = torch.stack(feat_list, dim=0)
        
    return image_features, attn_weights, all_feats


def reload_visual_attn(model, layers=6):
    model = model.visual.reload_self_attn(layers=layers)
    return model 
