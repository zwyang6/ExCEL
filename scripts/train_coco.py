...
parser.add_argument("--eval_iters", default=100, type=int, help="validation iters")
parser.add_argument("--warmup_iters", default=200, type=int, help="warmup_iters")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            name, inputs, cls_labels, _ = next(train_loader_iter)
        except:
            train_loader_iter = iter(train_loader)
            name,  inputs, cls_labels,_= next(train_loader_iter)
        ...
        # use_ex_fts = True if n_iter >= 30000 else False
        # attr_maps_raw = cure_attr_map_flip(model, inputs, use_ex_fts, flip=False, raw_fts=fts_diver)
        if n_iter >= 30000:
        ...
        for i, attr_map in enumerate(attr_maps_raw):
            ...
            seg_attn = attn_pred[i,...].unsqueeze(0) if n_iter >= 30000 else None
            refined_attr_maps, cls_lst = refine_cams_with_aff(attr_map, attn_weight, cls_label, size=inputs.shape[2:], seg_attn=seg_attn, caa_thre=0.88)
        ...
        ### diversity loss from ToCo, https://github.com/rulixiang/ToCo
        ...
        aff_mask = cams_to_affinity_label(aff_pseudos, mask=attn_mask)
        _, diver_pseudo_labels = lam_to_label(attr_maps_raw.permute(0,2,1).reshape(args.spg,(args.num_classes-1),mask_size,mask_size), cls_labels.cuda(), img_box=None, ignore_mid=False, bkg_thre=args.bkg_thre, high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index)
        ...
        if (n_iter + 1) % args.eval_iters == 0 and (n_iter + 1) >= 1:
            ...
            if args.local_rank == 0:
                ...
                if args.save_ckpt and (n_iter + 1) >= 40000:
                    torch.save(model.state_dict(), ckpt_name)
            tab_results = build_validation(model=model, par=par, val_loader=val_loader, num_classes=args.num_classes)
            if args.local_rank == 0:
                logging.info("\n"+tab_results)
...
if __name__ == "__main__":
    ...
    args.work_dir = os.path.join(args.work_dir, f'{args.dataset_name}', timestamp_1, exp_tag)
