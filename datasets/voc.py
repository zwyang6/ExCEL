...
class VOC12ClsDataset(VOC12Dataset):
    ...
    def __transforms(self, img, label):
        img = np.array(img)
        img,label = transforms.random_scaling(img, scale_range=self.rescale_range,label=label)
        img, label = transforms.random_fliplr(img, label)
        img, label, img_box = transforms.random_crop(img, label, crop_size=self.crop_size, mean_rgb=[0, 0, 0], ignore_index=self.ignore_index)
    ...
    def __getitem__(self, idx):
        ...
        image, label, img_box = self.__transforms(image, label)
