...
def refine_cams_with_bkg_weclip(cam_refined_list, inputs_denorm, cls_lst,par, h, w):
    ...
    with torch.no_grad():
        cam_labels = _refine_cams(par, inputs_denorm, cams, valid_key)

    return cam_labels



def refine_cams_with_aff(attr_maps, attn_weights, cls_label, size, attn_layers=6):

    h, w = size
    attn_weight = torch.cat(attn_weights, dim=0)
    attn_weight = attn_weight[:, 1:, 1:][-attn_layers:]
    ...
    cls_lst = torch.where(cls_label)[1]
    cam_refined_list = []
    all_cam_list = []

    # for attr_map in attr_maps:
    for i, cls in enumerate(cls_lst):
        grayscale_cam =attr_maps[:,:,cls].cpu().numpy().reshape(4, h // 16, w // 16)

        box, cnt = scoremap2bbox(scoremap=grayscale_cam, threshold=0.79, multi_contour_eval=True)
    ...
    return cam_refined_list, cls_lst