...
class EncoderTransformer(VisionTransformer):
    def __init__(self, layers=3, pretrained_cfg=None, *args, **kwargs):
        super().__init__(depth=layers, *args, **kwargs)
    ...
    def forward(self, x: torch.Tensor):
        ...
        for blk in self.blocks:
            x, weights = blk(x, attn_mask=None)
...
class Full_EncoderTransformer(VisionTransformer):
    def __init__(self, layers=12, pretrained_cfg=None, *args, **kwargs):
        super().__init__(depth=layers, *args, **kwargs)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.norm(x)[:, 1:]
        x = x.transpose(1, 2).reshape(b, x.shape[-1], h, w)

        return  x, weights


class MCTformer(VisionTransformer):
    ...
    def forward_features(self, x, attn_mask=None):
        ...
        x = self.norm(x)[:, self.num_classes:]
...
def matvit_base_patch16_224(pretrained=False, if_reload=True, **kwargs):
    model = MCTformer(
...
def reload2(model, num_attri=5):
    
    checkpoint = torch.load('/reference_codes/WeCLIP/pretrained/jx_vit_base_p16_224-80ecf9dd.pth', map_location='cpu')
    try:
    ...
    extra_tokens = pos_embed_checkpoint[:, :num_extra_tokens].repeat(1,(num_attri),1)
    ...
    new_cls_token = cls_token_checkpoint.repeat(1,(num_attri),1)
...
def reload(model, depth=3):
    ...
    for k, _ in state_dict.items():
        if 'blocks.' in k:
            layer = int(k.split('blocks.')[-1].split('.')[0])
            k_rename = k.replace('blocks.'+ str(layer),'blocks.'+ str(layer + 12 - depth))
            new_dict[k] = checkpoint_model[k_rename]
        elif k in checkpoint_model:
            new_dict[k] = checkpoint_model[k]
...
if __name__ == "__main__":
    model = Full_EncoderTransformer()
    reload(model=model, depth=12)